...
// scoreTaiwan:連買強勢股應排最前且觸發(含基本面欄位 → 摘要)
const twRows = [
  { code: "2330", name: "台積電", close: 1000, volume: 20_000_000, turnover: 2e10,
    ...
    industry: "半導體業", theme: "全球晶圓代工龍頭", revYoY: 35.2, revAccYoY: 22.1, revLabel: "2026/06", revMapLoaded: true, ytd: 12.34 },
...
const twRes = scoreTaiwan(twRows);
const tw = twRes.rows;
...
// 基本面寫入摘要
assert.ok(tw[0].summary.includes("半導體業"), "摘要應含產業別");
assert.ok(tw[0].summary.includes("全球晶圓代工龍頭"), "摘要應含題材標籤");
assert.ok(tw[0].summary.includes("營收年增35.2%"), "摘要應含營收年增率");
...
assert.equal(picked.code, "2317");
assert.equal(picked.of, 2);
assert.equal(picked.rank, 2);
assert.equal(scoreTaiwan(twRows, { pick: "8888" }).picked, null);
...
assert.ok(us[0].summary.includes("【題材】AI GPU"), "美股摘要應含題材");
assert.ok(us[0].summary.includes("成長42%"), "美股摘要應含EPS成長");

const usDeg = scoreUS(usRows, { valuationAvailable: false }).rows;
assert.equal(usDeg[0].valScore, 50);
assert.equal(usDeg[1].valScore, 50);
assert.equal(scoreUS(usRows, { pick: "intc" }).picked.code, "INTC", "pick應不分大小寫");
